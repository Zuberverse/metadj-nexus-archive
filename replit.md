...
**Last Modified**: 2026-01-13 14:19 EST
...
Database: JSON data files (current); Replit-managed database (planned)
Authentication: Required for app UI (media streaming routes are public)
```

## Database & Hosting (Replit)

### Current State (JSON Data Files)
- Primary data lives in `src/data/music.json` and `src/data/collections.json`
- Versioned in Git alongside the app code
- No external database credentials required

### Managed Database (Planned)
When a managed database is introduced, host it within Replit and document the details here.

**Required Secrets (once enabled):**
- `DATABASE_URL` — connection string for the managed database

**Optional (provider-specific):**
- `DATABASE_SSL` — set to `true` if SSL is required
- `DATABASE_POOL_SIZE` — connection pool limit (document final value)

**Operational Notes (fill in when enabled):**
- Engine + plan tier:
- Region:
- Migration command:
- Backup cadence:
...
1. Provision the managed database in Replit
2. Store `DATABASE_URL` (and any provider-required settings) in Replit Secrets
3. Run migrations and verify schema health
4. Confirm backup exports and restore path
5. Update this section with the final provider details
...
**Managed Database (Planned)**:
- Document the provider, export format, and restore command here once enabled
- Store backups alongside the media backup workflow
